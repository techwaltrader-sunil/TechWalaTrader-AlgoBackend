...
const algoTradeLogSchema = new mongoose.Schema({
    ...
    brokerName: { type: String, required: true },
    symbol: { type: String, required: true },
    action: { type: String, required: true }, // BUY ya SELL ya SYSTEM_UPDATE
    quantity: { type: Number, required: true },
    
    // 🔥 THE FIX: Yahan 'INFO' aur 'REJECTED' add kar diya gaya hai 🔥
    status: { type: String, enum: ['SUCCESS', 'FAILED', 'INFO', 'REJECTED'], required: true },
    
    message: { type: String }, // Error aane par error message, success/info par message
    orderId: { type: String }, // Dhan ka aayega agar success hua to
    createdAt: { type: Date, default: Date.now }
}, { 
    // 🔥 Mongoose ko explicitly bata rahe hain ki collection ka naam kya rakhna hai
