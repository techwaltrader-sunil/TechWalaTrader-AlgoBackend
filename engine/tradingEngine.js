...
//                     const broker = await Broker.findById(deployment.brokers[0]);
//                     if (broker && broker.engineOn) {
//                         // 🔥 THE FIX: Scanner ko batao ki Spot chahiye ya Future!
//                         const underlyingType = config.underlying || "Spot";
//                         const signal = await getIndicatorSignal(strategy, broker, baseSymbol, underlyingType);
...
//                 const broker = await Broker.findById(deployment.brokers[0]);
//                 if (broker && broker.engineOn) {
//                     // 🔥 THE FIX: Exit Scanner ko bhi Underlying Type pass karo
//                     const underlyingType = config.underlying || "Spot";
//                     const shouldExit = await getIndicatorExitSignal(strategy, broker, baseSymbol, deployment.signalType || "NONE", underlyingType);
...
//             // ==============================================================
//             // ⏰ 3. TIME-BASED AUTO SQUARE-OFF LOGIC (BTST UPGRADED)
//             // ==============================================================
//             if (squareOffTime && hasActiveLegs && !executionLocks.has(exitLockKey)) {
                
//                 // 🔥 BTST SHIELD LOGIC
//                 const orderType = (config.orderType || strategy.data?.orderType || "MIS").toUpperCase();
//                 const todayStr = moment().tz("Asia/Kolkata").format("YYYY-MM-DD");
                
//                 // Deployment/Trade kab enter hua tha
//                 const entryDateStr = moment(deployment.createdAt).tz("Asia/Kolkata").format("YYYY-MM-DD");
                
//                 let isEligibleForSquareOff = true;
                
//                 // Agar BTST trade hai aur aaj hi entry li hai, to aaj Square-off block kardo!
//                 if (orderType === 'BTST' && todayStr === entryDateStr) {
//                     isEligibleForSquareOff = false;
//                 }

//                 // Agar exit karne ki permission hai (MIS hai, ya BTST ka agla din aa gaya hai)
//                 if (isEligibleForSquareOff) {
//                     const [currH, currM] = currentTime.split(':').map(Number);
//                     const currentMinutes = (currH * 60) + currM;
...
//                     }
//                 } else {
//                     // Sirf console log for debugging, taki aapko pata rahe ki BTST shield active hai
//                     // console.log(`🛡️ BTST Shield Active: Square-off blocked for today for ${strategy.name}`);
//                 }
//             }
...
//             // ==============================================================
//             if (hasActiveLegs && (deployment.status === 'ACTIVE' || deployment.status === 'PARTIALLY_COMPLETED')) {
                
//                 // 🟢 1. MTM Check (Sabse pehle yahi chale)
//                 await handleMtmSquareOff(deployment, strategy, executionLocks, exitLockKey);

//                 // 🔥 VITAL FIX: Agar MTM ne exit maar diya hai, to neeche wala trailing/SL logic mat chalao
//                 // Isse aapka 'double exit' ka risk khatam ho jayega
//                 if (executionLocks.has(exitLockKey)) {
//                     console.log("🛑 MTM Exit active, skipping individual trailing/SL logic.");
//                     continue; // Deployment loop mein agla deployment dekhega
//                 }

//                 const broker = await Broker.findById(deployment.brokers[0]); 
//                 if (broker && broker.engineOn) { // broker check ko yahan rakhein
                    
//                     // 🔄 Refresh current state to avoid stale data
//                     const currentDeployment = await Deployment.findById(deployment._id);
                    
//                     for (let i = 0; i < currentDeployment.executedLegs.length; i++) {
//                         let currentLeg = currentDeployment.executedLegs[i];
...
//                         await sleep(1000); 
//                         let liveLtp = await fetchLiveLTP(broker.clientId, broker.apiSecret, currentLeg.exchange, currentLeg.securityId);
                        
//                         if (!liveLtp || liveLtp <= 0) {
//                             await sleep(500); // आधा सेकंड रुको
...
//                                 // 🔥🔥🔥 THE MAIN FIX: YAHAN LTP UPDATE MISSING THA! 🔥🔥🔥
//                                 currentLeg.ltp = liveLtp;
//                                 currentLeg.liveLtp = liveLtp; 
