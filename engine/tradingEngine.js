...
//                                 // 2. Premium Fetch Callback (Live API) - THE ULTIMATE FIX 🚀
//                                 const liveFetchPremiumCallback = async (optType, expectedStrike) => {
                                    
//                                     // 1️⃣ SOLID SLEEP: API को ब्लॉक (429) होने से बचाने के लिए 300ms का ब्रेक
//                                     await sleep(300);

//                                     // 2️⃣ DYNAMIC STRIKE LOGIC: सीधा "Strike" मोड यूज़ करें! 
//                                     let inst = getOptionSecurityId(
//                                         baseSymbol, 
//                                         currentSpotPrice, 
//                                         "Strike",                  // 👈 Criteria: "Strike" 
//                                         expectedStrike.toString(), // 👈 Exact Strike (e.g. "23800")
//                                         optType, 
//                                         strategy.data?.legs[0]?.expiry || "WEEKLY"
//                                     ); 

//                                     if (!inst || !inst.id) return null;
                                    
//                                     // सेफ्टी चेक: कहीं गलत स्ट्राइक तो नहीं आ गई?
//                                     if (inst.strike && Number(inst.strike) !== Number(expectedStrike)) {
//                                         return null; 
//                                     }

//                                     // 3️⃣ असली OTM/ITM टोकन मिलने के बाद Live LTP फेच करो
//                                     const ltp = await fetchLiveLTP(broker.clientId, broker.apiSecret, inst.exchange, inst.id);
                                    
//                                     // 4️⃣ पूरा 'inst' ऑब्जेक्ट और 'id' रिटर्न करो!
//                                     if (ltp) return { price: ltp, strike: expectedStrike, inst: inst, id: inst.id };
...
//                                     // 🔥 THE WEBSOCKET FIX: Sirf valid numerical IDs ko hi filter karo
//                                     const liveTokens = legsToExecute
//                                         .map(leg => leg.inst && leg.inst.id ? String(leg.inst.id) : null)
//                                         .filter(id => id && id !== 'undefined' && !id.includes('ATM') && !id.includes('OTM'));

//                                     // Agar kam se kam 1 valid token hai, tabhi subscribe karo
//                                     if (liveTokens.length > 0) {
//                                         setTimeout(() => {
//                                             dhanStreamer.subscribeTokens("NSE_FNO", liveTokens);
//                                             console.log(`📡 WebSocket Subscribed to valid tokens: ${liveTokens.join(', ')}`);
//                                         }, 2000); 
//                                     } else {
//                                         console.log(`⚠️ No valid tokens found for WebSocket Subscription.`);
//                                     }
//                                     // =========================================================
