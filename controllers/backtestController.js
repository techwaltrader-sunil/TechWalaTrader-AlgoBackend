...
//                 // 2. TICK EVALUATION (With Bulletproof Local PnL & Theta)
//                 // 2. TICK EVALUATION (With Real API Exit Fetch)
//                 if (ratioEngine.status === 'ACTIVE' || ratioEngine.status === 'RECOVERY_MODE') {
                    
//                     const spotChange = spotClosePrice - ratioEngine.activeLegs[0].strike; 
//                     const minutesPassed = Math.max(0, timeInMinutes - currentStartMin);
//                     const decayFactor = Math.min(1, minutesPassed / 375); 
                    
//                     // (A) Simulated LTPs for intra-minute tracking (Boundary & SL Checks)
//                     let mockLTPs = {};
...
//                         } else {
//                             delta = leg.type === 'CE' ? 0.25 : -0.25;
//                             thetaDecay = leg.entryPrice * 0.5 * decayFactor;
//                         }
...
//                         if (decision && decision.action === 'RECOVERY_SWITCH') {
//                             const recoveryPnL = await executeRealExit("STOPLOSS_TO_RECOVERY", currentTimeStr); 
                            
//                             console.log(`\n🚑 [RECOVERY TRIGGERED] Main Trade SL Hit! Switching to Firefighting Mode at ${currentTimeStr} | Spot: ₹${spotClosePrice}`);
//                             console.log(`   📉 Realized Loss booked: ₹${recoveryPnL.toFixed(2)}`);
                            
//                             dailyBreakdownMap[dateStr].tradesList.push({
//                                 symbol: "RATIO_SPREAD_MAIN",
//                                 pnl: recoveryPnL,
//                                 exitType: "STOPLOSS_TO_RECOVERY",
//                                 exitTime: `${dateStr} ${currentTimeStr}:00`
//                             });
//                             dailyBreakdownMap[dateStr].pnl += recoveryPnL;
//                             dailyBreakdownMap[dateStr].trades += 1;
//                         }
                        
//                         if (decision && decision.action === 'EXIT_ALL') {
