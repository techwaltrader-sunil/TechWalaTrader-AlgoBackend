...
//             const paSignal = checkPriceActionSignal(recentHtfSlice, ltfSlice, setupType);
...
//             // DEBUG लॉग
//             if(i % 100 === 0) {
//                 console.log(`🔎 SMC Sync | Time: ${dateStr} ${h}:${m} | HTF Candles: ${recentHtfSlice.length} | L=${priceActionLongSignal} S=${priceActionShortSignal}`);
//             }
//         }
...
//             finalSmcSignals = identifyMechanicalStructure(cachedHtfData);
//         }
    ...
    try {
       ...
       // 🔥 NAYA PRICE ACTION LOGIC (MULTI-TIMEFRAME SYNC)
        ...
        if (strategy.type === "Price Action Based" || strategy.data?.type === "Price Action Based") {
            ...
            const paSettings = strategy.data?.priceActionSettings || {};
            const setupType = paSettings.setupType || "BOS (Break of Structure)";
            // 🔥 FIX: डेटाबेस से यूज़र का चुना हुआ ट्रेंड निकालें
            const userChosenTrend = paSettings.startingTrend || "AUTO"; 
            
            // 🔥 FIX: checkPriceActionSignal को भी userChosenTrend भेजें
            const paSignal = checkPriceActionSignal(recentHtfSlice, ltfSlice, setupType, userChosenTrend);
        ...
        let finalSmcSignals = [];
        if (strategy.type === "Price Action Based" || strategy.data?.type === "Price Action Based") {
            // 🔥 FIX: यहाँ भी यूज़र का चुना हुआ ट्रेंड निकालें
            const paSettings = strategy.data?.priceActionSettings || strategy.priceActionSettings || {};
            const userChosenTrend = paSettings.startingTrend || "AUTO";
            
            // चार्ट पर दिखाने के लिए पूरे 1 महीने के HTF (Manager) सिग्नल्स जनरेट करें
            finalSmcSignals = identifyMechanicalStructure(cachedHtfData, userChosenTrend); // 🔥 यहाँ पास कर दें!
        }
