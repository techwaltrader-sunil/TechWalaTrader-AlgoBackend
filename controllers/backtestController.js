       ...
       const calculateTargetStrike = (atmStrike, legObj, optionType) => {
        ...
        // 🔥 NAYA PRICE ACTION LOGIC (MULTI-TIMEFRAME SYNC)
            ...
            if (isRatioSpreadStrategy && ratioEngine) {
                ...
                // 🔥 THE FIX: continue को ब्रैकेट के अंदर ले लिया गया है
                continue; // 🔥 ISE BILKUL NAHI HATANA HAI!
            } // <--- Ratio Spread का मेन ब्लॉक यहाँ बंद होना चाहिए!
            ...
            // 2. Agar Indicator-Based strategy hai...
            else {
                // 🔥 FIX: BTST ऑर्डर्स के लिए दिन भर के सिग्नल्स को ब्लॉक मत करो। 
                // अगर orderType 'BTST' है, तो इंडिकेटर के सिग्नल्स को जिन्दा रहने दो ताकि 15:15 पर एंट्री मिल सके।
                if (timeInMinutes < currentStartMin && orderType !== "BTST") {
                    longSignal = false;
                    shortSignal = false;
                }
            }

            // 🔥 THE CULPRIT KILLED FIX: 
            if (strategy.type !== "Price Action Based" && strategy.data?.type !== "Price Action Based") {
                finalLongSignal = (txnType === 'Both Side' || txnType === 'Only Long' || isTimeBased) ? longSignal : false;
                finalShortSignal = (txnType === 'Both Side' || txnType === 'Only Short') ? shortSignal : false;
            }
            ...
            // 🛑 GATEKEEPER BLOCK: अगर 9:15 (Start Time) से पहले का सिग्नल है, तो उसे रोक दो
            // 🔥 FIX: BTST ऑर्डर्स के लिए दिन भर के सिग्नल्स को ब्लॉक मत करो
            if (timeInMinutes < currentStartMin && !isTimeBased && orderType !== "BTST") {
                finalLongSignal = false;
                finalShortSignal = false;
            }
            ...
            if (openTrades.length > 0) {
                ...
                for (let trade of openTrades) {
                    ...
                    if (orderType === "MIS") {
                        ...
                        // 🔥 NAYA FEATURE: UI से आने वाला मोड (अगर कुछ नहीं आया तो 'Next Day' डिफ़ॉल्ट रहेगा)
                        const btstExitMode = strategy.data?.config?.btstExitMode || strategy.config?.btstExitMode || "Next Day";

                        if (btstExitMode === "Expiry Day") {
                            // 🎯 EXPIRY DAY SQUARE OFF LOGIC
                            // एक्सपायरी की सही तारीख निकालें
                            const actualTradeExpiryStr = getNearestExpiryString(dateStr, upperSymbol, trade.legConfig?.expiry || "WEEKLY");
                            const tradeDTE = getTradingDaysToExpiry(istDate, actualTradeExpiryStr);

                            // अगर आज एक्सपायरी का दिन है (DTE 0), तो यूज़र के तय किए गए समय पर एग्ज़िट करो
                            if (tradeDTE === 0 && (timeInMinutes >= nextDayExitMin || isLastCandleOfDay)) {
                                forceSquareOff = true;
                                trade.exitReason = "BTST_EXPIRY_EXIT";
                            } 
                            // Fallback: अगर मार्केट बंद होने वाला है और DTE नेगेटिव में चला गया है
                            else if (tradeDTE < 0 && isLastCandleOfDay) {
                                forceSquareOff = true;
                                trade.exitReason = "BTST_EXPIRY_EXIT";
                            }
                        } 
                        else {
                            // 🎯 NEXT DAY SQUARE OFF LOGIC (पुराना डिफ़ॉल्ट लॉजिक)
                            // अगर आज का दिन एंट्री वाले दिन से अलग है, मतलब अगला दिन आ गया है
                            if (currentDateFormatted !== tradeEntryDate) {
                                if (timeInMinutes >= nextDayExitMin || isLastCandleOfDay) {
                                    forceSquareOff = true;
                                    trade.exitReason = "BTST_EXIT";
                    ...
                    if (trade.markedForExit || forceSquareOff) {
                        ...
                        // 🔴 THE SNIPER GATEKEEPER
                        // =========================================================================
                        const needsMarketPrice = ["MAX_LOSS", "MAX_PROFIT", "TIME_SQUAREOFF", "EOD_SQUAREOFF", "BTST_EXIT","BTST_EXPIRY_EXIT", "INDICATOR_EXIT", "STOPLOSS", "TARGET", "TRAILING_SL", "SL_MOVED_TO_COST", "LOCK_FIX_PROFIT", "LOCK_AND_TRAIL", "LEG_TRAIL_SL"].includes(trade.exitReason) || String(trade.exitReason).startsWith("EXIT_ALL");
                        let fakeTriggerRejected = false;
                            ...
                            if (foundExactExit && exitData) {
                                ...
                                if (!isValidTrigger || isFlatline) {
                                    ...
                                    if (["MAX_LOSS", "MAX_PROFIT"].includes(trade.exitReason)) {
                                        ...
                                        // 🔥 NAYA FIX: BTST_EXPIRY_EXIT को जोड़ा और null से बचने के लिए (cOpen || cClose) का Fallback लगाया
                                        trade.exitPrice = (trade.exitReason === "TIME_SQUAREOFF" || trade.exitReason === "BTST_EXIT" || trade.exitReason === "BTST_EXPIRY_EXIT" || String(trade.exitReason).startsWith("EXIT_ALL")) ? (cOpen || cClose) : (cClose || cOpen);
                                    }
                                }

                                // 🛡️ THE ULTIMATE NULL CATCHER: किसी भी हाल में exitPrice null नहीं होना चाहिए
                                if (trade.exitPrice === null || trade.exitPrice === undefined || isNaN(trade.exitPrice)) {
                                    trade.exitPrice = cOpen || mathPrice || spotClosePrice || trade.entryPrice;
                                }
                            }
                ...
                confirmedOtherExits.forEach(trade => {
                    ...
                    // 🛡️ SAFEGUARD: अगर किसी भी वजह से exitPrice null रह गया हो, तो इंजन क्रैश नहीं होगा
                    if (trade.exitPrice === null || trade.exitPrice === undefined || isNaN(trade.exitPrice)) {
                        trade.exitPrice = trade.entryPrice || 0; 
                        console.log(`⚠️ [WARNING] Exit Price null मिला ${trade.symbol} के लिए। Fallback इस्तेमाल किया गया!`);
                    }
            ...
            // 🔥 THE ROLLOVER FIX: Multi-day hold me daily entry block karo
            const currentBtstMode = strategy.data?.config?.btstExitMode || strategy.config?.btstExitMode || "Next Day";

            let canTakeNewEntry = openTrades.length === 0;

            if (orderType !== "MIS" && isTimeBased) {
                // अगर BTST है और Expiry Day तक होल्ड कर रहे हैं, तो स्टैकिंग (मल्टीपल एंट्री) ब्लॉक करो!
                if (orderType === "BTST" && currentBtstMode === "Expiry Day") {
                    canTakeNewEntry = openTrades.length === 0;
                } else {
                    canTakeNewEntry = true; // Normal CNC या Normal BTST rollover allow करेगा
                }
            }
...
//                     // 🟢 STEP 3: The Deep-Sea Scanner (Find Symmetric Strike with minimum error)
...
//                         if(!ceData || !peData) {
//                             console.log(`   ⚠️ Step ${step} Skipped: API didn't return data for strikes ${expectedCeStrike} / ${expectedPeStrike}`);
//                             continue;
//                         }
...
//                         // Optimization: Stop scanning if premiums are too small
...
//                         { strike: finalOtmCe.strike, type: 'CE', action: 'SELL', entryPrice: finalOtmCe.price, lots: 4, tag: 'MAIN', inst: { id: `CE_OTM`, lotSize: realLotSize } },
//                         { strike: finalOtmPe.strike, type: 'PE', action: 'SELL', entryPrice: finalOtmPe.price, lots: 4, tag: 'MAIN', inst: { id: `PE_OTM`, lotSize: realLotSize } }
//                     ];
                    
//                     ratioEngine.tradeBoundaries = ratioEngine.calculateBreakevens();
                    
//                     // 🔥 🟢 STEP 5: SMART ESTIMATED MARGIN CALCULATOR (Stockmock & SEBI Expiry Style)
//                     let totalBuyQtyCE = 0, totalSellQtyCE = 0;
//                     let totalBuyQtyPE = 0, totalSellQtyPE = 0;

//                     // 1. Qty count karo CE aur PE ka alag-alag
//                     ratioEngine.activeLegs.forEach(leg => {
//                         const legQty = leg.lots * (leg.inst?.lotSize || 65);
//                         if (leg.type === 'CE') {
//                             if (leg.action === 'BUY') totalBuyQtyCE += legQty;
//                             if (leg.action === 'SELL') totalSellQtyCE += legQty;
//                         } else if (leg.type === 'PE') {
//                             if (leg.action === 'BUY') totalBuyQtyPE += legQty;
//                             if (leg.action === 'SELL') totalSellQtyPE += legQty;
//                         }
//                     });

//                     // 2. Hedged aur Naked Qty nikalo (1:4 ratio me 1 Hedged, 3 Naked hota hai)
//                     const hedgedQtyCE = Math.min(totalBuyQtyCE, totalSellQtyCE);
//                     const nakedQtyCE = totalSellQtyCE > totalBuyQtyCE ? totalSellQtyCE - totalBuyQtyCE : 0;
                    
//                     const hedgedQtyPE = Math.min(totalBuyQtyPE, totalSellQtyPE);
//                     const nakedQtyPE = totalSellQtyPE > totalBuyQtyPE ? totalSellQtyPE - totalBuyQtyPE : 0;

//                     // 3. Realistic Margin Rates (Per QTY)
//                     const isBank = upperSymbol.includes("BANK");
//                     // Nifty Naked: ~₹1,43,000 per lot (65 qty) = ~₹2200 per qty
//                     // Nifty Hedged: ~₹29,000 per lot (65 qty) = ~₹450 per qty
//                     const nakedRate = isBank ? 3200 : 2200; 
//                     const hedgedRate = isBank ? 600 : 450;

//                     let estMargin = (nakedQtyCE * nakedRate) + (hedgedQtyCE * hedgedRate) + 
//                                     (nakedQtyPE * nakedRate) + (hedgedQtyPE * hedgedRate);

//                     // 🔥 SEBI Expiry Day Rule Impact (Additional ELM & Tightening)
//                     // Expiry ke din margin spike hota hai, isliye hum ek 5% ka safety buffer add kar rahe hain
//                     estMargin = estMargin * 1.05; 
...
//                     let mockLTPs = {};
//                     ratioEngine.activeLegs.forEach(leg => {
//                         let delta = 0; let thetaDecay = 0;
//                         if (leg.tag === 'MAIN') {
//                             if (leg.type === 'CE' && leg.action === 'BUY') delta = 0.5;
//                             if (leg.type === 'PE' && leg.action === 'BUY') delta = -0.5;
//                             if (leg.type === 'CE' && leg.action === 'SELL') delta = 0.15;
//                             if (leg.type === 'PE' && leg.action === 'SELL') delta = -0.15;
//                             thetaDecay = leg.action === 'SELL' ? (leg.entryPrice * 0.8 * decayFactor) : (leg.entryPrice * 0.4 * decayFactor);
//                         } else {
//                             // 🔥 FIX: RECOVERY TRADE (ATM Sells) THE REAL GREEKS
//                             // Recovery hamesha ATM hoti hai, isliye iska Delta 0.50 hoga!
//                             delta = leg.type === 'CE' ? 0.50 : -0.50;
                            
//                             // Recovery me Theta decay thoda aggressive rakhenge (Expiry ke hisaab se)
//                             thetaDecay = leg.entryPrice * 0.6 * decayFactor;
//                         }
//                         mockLTPs[leg.inst?.id] = Math.max(0.05, (leg.entryPrice + (spotChange * delta)) - thetaDecay);
//                     });
...
//                             if (realExitPrice === null) {
//                                 realExitPrice = mockLTPs[leg.inst?.id] !== undefined ? mockLTPs[leg.inst?.id] : leg.entryPrice;
//                                 console.log(`   ⚠️ API Failed for Leg ${idx+1}. Using simulated price: ₹${realExitPrice.toFixed(2)}`);
//                             }

//                             const mult = leg.lots * (leg.inst?.lotSize || 25);
//                             const legPnL = leg.action === 'BUY' ? (realExitPrice - leg.entryPrice) * mult : (leg.entryPrice - realExitPrice) * mult;
//                             finalRealPnL += legPnL;
...
//                         const decision = ratioEngine.evaluateTick(currentTimeStr, mockLTPs, spotClosePrice);
...
//                                 // const realLotSize = Number(instrumentData.lotSize) || Number(strategyLegs[0]?.quantity) || 65;
//                                 // ratioEngine.activeLegs = [
//                                 //     { strike: recoveryAtmStrike, type: recoveryType, action: 'SELL', entryPrice: recoveryEntryPrice, lots: 4, tag: 'RECOVERY', inst: { id: `${recoveryType}_RECOVERY`, lotSize: realLotSize } }
//                                 // ];
                                
//                                 // console.log(`   🔥 [RECOVERY ENTRY] Sold 4 Lot(s) ${recoveryType} @ Strike ${recoveryAtmStrike} | Premium: ₹${recoveryEntryPrice.toFixed(2)} | Risk Allocated: -₹${recoveryRiskBudget.toFixed(2)}`);
