...
//     try {
//         const query = `INSERT INTO strategy_profiles (profile_name, config) VALUES ($1, $2) RETURNING *`;
//         const result = await pgPool.query(query, [profileName, config]); // ✅ pgPool use kiya hai
...
//     try {
//         const query = 'SELECT * FROM strategy_profiles ORDER BY id DESC';
//         const result = await pgPool.query(query); // ✅ pgPool use kiya hai
...
//             // 🎯 THE FIX: Dhan का डेटा चाहे किसी भी फॉर्मेट में आये, यह उसे पकड़ लेगा
//             const apiData = spotRes.data?.data || spotRes.data;

//             if (apiData && apiData.close && apiData.close.length > 0) {
//                 const closePrices = apiData.close;
//                 liveSpotPrice = Number(closePrices[closePrices.length - 1]);
//             } else {
//                 console.log("⚠️ Dhan API returned empty data for spot:", spotRes.data);
//             }
//         } catch (spotErr) {
//             // 🎯 THE DEBUGGER: अब यह असली एरर छापेगा कि धन API ने रिक्वेस्ट क्यों रिजेक्ट की!
//             const realError = spotErr.response ? JSON.stringify(spotErr.response.data) : spotErr.message;
//             console.error("⚠️ Plan A Failed (Intraday API):", realError);
//         }
